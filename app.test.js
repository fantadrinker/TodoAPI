...
describe("GET /todos", () => {
  beforeEach(async () => {
    ...
    await db.none(`insert into todoitems(id, value, completed) values('1', 'b: todo', false)`);
    await db.none(`insert into todoitems(id, value, completed) values('2', 'a: todo', false)`);
    await db.none(`insert into todoitems(id, value, completed, completed_time) values('3', 'c: todo', true, now())`);
    await db.none(`insert into todoitems(id, value, completed, completed_time) values('4', 'd: todo', true, now() + interval '1 day')`);
    await db.none(`insert into todoitems(id, value, completed, completed_time) values('5', 'e: todo', true, now() + interval '2 day')`);
  });
  afterEach(async () => {
    await db.none(`delete from todoitems`);
  });
  ...
  it("should return max 10 results when completed is true", async () => {
    await db.none(`delete from todoitems`);
    for (let i = 0; i < 15; i++) {
      await db.none(`insert into todoitems(id, value, completed, completed_time) values('${i}', 'todo', true, now())`);
    }
    const completed = await request(app).get('/todos?completed=true');
    const completedData = completed.body.data;
...
describe("PUT /todos/:id", () => {
  ...
  it("should update completed to todo with id", async () => {
    const response = await request(app).put('/todos/3').send({completed: false});
    expect(response.statusCode).toBe(200);
